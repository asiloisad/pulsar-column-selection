...
export default {
  ...
  activate() {
    ...
    this.stickyFlag = false ; this.pickerFlag = false ; this.blockSelectionStatus = null
  },
  ...
  sticky() {
    ...
    this.updateStyle(this.stickyFlag)
  ...
  initTest (e) {
    if (e.which!==this.mouseButton) {
      ...
      return this.stickyFlag || e.shiftKey
    } else if (this.selectKey===2) {
      return this.stickyFlag || e.altKey
    } else if (this.selectKey===3) {
      return this.stickyFlag || e.ctrlKey
  ...
  handleLoad(editor) {
    ...
    const onContextMenu = (e) => {
      if (this.pickerFlag || (this.initTest(e) & !context)) {
        e.preventDefault() ; e.stopPropagation() ; this.pickerFlag = false
        if (this.blockSelectionStatus) {
          this.blockSelectionStatus.statusPicker(this.pickerFlag)
        }
        return false
