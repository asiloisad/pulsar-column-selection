...
export default {
  ...
  activate() {
    ...
    this.stickyFlag = false ; this.pickerFlag = 0 ; this.blockSelectionStatus = null
  },
  ...
  picker() {
    this.pickerFlag = this.pickerFlag===0 ? 1 : 0
    if (this.blockSelectionStatus) {
  ...
  handleLoad(editor) {
    ...
    const onContextMenu = (e) => {
      if (this.pickerFlag || (this.initTest(e) & !context)) {
        e.preventDefault() ; e.stopPropagation() ; this.pickerFlag = 0
        if (this.blockSelectionStatus) {
          this.blockSelectionStatus.statusPicker(this.pickerFlag)
        }
        return false
