...
module.exports = {
  ...
  screenPositionForMouseEvent(e) {
    ...
    let row = this.editor.component.screenPositionForPixelPosition(
      pixelPosition
    ).row;
  ...
  selectBox(e) {
    ...
    if (this.mouseStart && this.mouseEnd) {
      ...
      for (
        let { row } = this.mouseStart,
          end = this.mouseEnd.row,
          asc = this.mouseStart.row <= end;
        asc ? row <= end : row >= end;
        asc ? row++ : row--
      ) {
        ...
        if (range.isEmpty()) {
          let pointStart = this.editor.screenPositionForBufferPosition(
            range.start
          );
      ...
      for (let selection of selections.slice(
        localRanges.length + this.selections.length
      )) {
        selection.destroy();
      }
      ...
      this.editor.mergeIntersectingSelections({}, () => {
        ...
        for (let i = 0; i < localRanges.length; i++) {
          if (selections[i + this.selections.length]) {
            selections[i + this.selections.length].setBufferRange(
              localRanges[i],
              { reversed }
            );
          } else {
