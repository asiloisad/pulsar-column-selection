...
export default {
  ...
  activate() {
    ...
    window.addEventListener('mousedown', this.mouseDown, { capture: true })
    window.addEventListener('mouseup', this.mouseUp, { capture: true })
    window.addEventListener('mousemove', this.mouseMove, { capture: true })
    window.addEventListener('contextmenu', this.contextMenu, { capture: true })
  },

  deactivate () {
    window.removeEventListener('mousedown', this.mouseDown, { capture: true })
    ...
    window.removeEventListener('contextmenu', this.contextMenu, { capture: true })
    ...
    if (this.pickerFlag) {
      if (!this.findEditor(e)) {
        ...
        if (this.pickerFlag===1) {
          ...
          this.selectBox(e) ; this.picker() ; this.reset()
        } else {
          this.pickerFlag = 1
  ...
  selectBox(e) {
    ...
    if (this.props.mouseStart && this.props.mouseEnd) {
      ...
      for (let i=0 ; i<ranges.length ; i++) {
        if (i===0) {
          this.editor.setSelectedBufferRange(ranges[i], { reversed:revers[i] })
        } else {
          this.editor.addSelectionForBufferRange(ranges[i], { reversed:revers[i] })
        }
      }
    }
  },
